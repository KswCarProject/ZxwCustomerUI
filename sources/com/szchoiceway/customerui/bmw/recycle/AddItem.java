...
public class AddItem extends RecycleItemBase {
    ...
    public String getTag() {
        return MainPageUIController.ADD_TAG;
    }
    ...
    public View getSetView(Context context, boolean z) {
        ...
        if (this.mSmallMode == z && (view = this.mAddView) != null) {
            return view;
        }
        this.mSmallMode = z;
        if (this.m_iModeSet == 20) {
            if (z) {
                this.mAddView = LayoutInflater.from(context).inflate(R.layout.pemp_bmw_id8_add_item_layout_small, (ViewGroup) null);
            } else {
                this.mAddView = LayoutInflater.from(context).inflate(R.layout.pemp_bmw_id8_add_item_layout, (ViewGroup) null);
            }
        } else if (z) {
            this.mAddView = LayoutInflater.from(context).inflate(R.layout.bmw_id8_add_item_layout_small, (ViewGroup) null);
        } else {
            this.mAddView = LayoutInflater.from(context).inflate(R.layout.bmw_id8_add_item_layout, (ViewGroup) null);
        }
        init();
    ...
    private void init() {
        ImageView imageView = (ImageView) this.mAddView.findViewById(R.id.imgBk);
        this.imgBk = imageView;
        if (imageView != null) {
            imageView.setVisibility(this.mSmallMode ? 8 : 0);
        }
        this.imgLine = this.mAddView.findViewById(R.id.imgLine);
        this.mTitle = (TextView) this.mAddView.findViewById(R.id.title);
        updateInfo();
    }
    ...
    public void updateInfo() {
        ...
        if (this.mTitle != null) {
            this.mTitle.setText(this.mContext.getString(R.string.lb_add_widget).toUpperCase());
        }
        if (this.m_iModeSet == 20) {
            int i = this.mMode;
            if (i == 0) {
