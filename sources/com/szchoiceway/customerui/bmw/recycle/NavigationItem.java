...
public class NavigationItem extends RecycleItemBase {
    ...
    public View getSetView(Context context, boolean z) {
        ...
        if (this.mSmallMode == z && (view = this.mNaviView) != null) {
            return view;
        }
        this.mSmallMode = z;
        if (this.m_iModeSet == 20) {
            if (z) {
                this.mNaviView = LayoutInflater.from(context).inflate(R.layout.pemp_bmw_id8_navi_item_layout_small, (ViewGroup) null);
            } else {
                this.mNaviView = LayoutInflater.from(context).inflate(R.layout.pemp_bmw_id8_navi_item_layout, (ViewGroup) null);
            }
        } else if (z) {
            this.mNaviView = LayoutInflater.from(context).inflate(R.layout.bmw_id8_navi_item_layout_small, (ViewGroup) null);
        } else {
            this.mNaviView = LayoutInflater.from(context).inflate(R.layout.bmw_id8_navi_item_layout, (ViewGroup) null);
        }
        init();
    ...
    private void init() {
        ImageView imageView = (ImageView) this.mNaviView.findViewById(R.id.imgBk);
        this.imgBk = imageView;
        if (imageView != null) {
            imageView.setVisibility(this.mSmallMode ? 8 : 0);
        }
        this.imgLine = this.mNaviView.findViewById(R.id.imgLine);
        this.mTitle = (TextView) this.mNaviView.findViewById(R.id.title);
        updateInfo();
    }

    public void updateInfo() {
        if (this.mTitle != null) {
            this.mTitle.setText(this.mContext.getString(R.string.lb_navi2).toUpperCase());
        }
